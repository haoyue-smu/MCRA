...
function CourseBrowser({ cart, addToCart, removeFromCart }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-smu-blue mb-2">Course Browser</h1>
        <p className="text-gray-600">Browse courses with live demand transparency and S/U indicators</p>
      </div>
                  ...
                  <div className="space-y-2">
                    {selectedCourse.assessments.map((a, i) => (
                      <div key={i} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                        <span>{a.type}</span>
                        <span className="text-sm text-gray-600">{a.date} ({a.weight}%)</span>
                      </div>
                    ))}
                  </div>
                </div>
